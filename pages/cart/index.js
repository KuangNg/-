...
Page({
  ...
  onShow: function () {
    ...
    // every 数组函数 循环遍历，全部为true才为true
    // cartGoods.length >0? 为什么要判断这个呢？ 因为缓存可以为空数组。而every对于空数据是看为true的，展示不对的，所以要cartGoods有数据才判断
    // let allChecked = cartGoods.length >0?cartGoods.every(v => v.checked):false 
    this.setData({
      userAddress: Address
    })
    this.setCart(cart)
  ...
  async handleChooseAddress() {
    ...
    // 简化以上代码 用 es7语法
    try {
      // 1. 先获取 权限状态 确定 true  第一次 underfine  取消 false
      const res = await getSetting()
      const scopedAddress = res.authSetting["scope.address"]
      // 2 判断用户的权限状态  取消 false   
  ...
  // 设置购物车商品状态同时 重新计算 底部工具栏的数据 全选 总价格 购买的数量
  setCart(cart) {
    let allChecked = true;
    let totalPrice = 0;
    let totalNum = 0;
  ...
  async handleItemNumEdit(e) {
    ...
    // 4 判断是否要执行删除
    if (cart[index].num === 1 && use === -1) {
      // 4.1 弹窗提示
      const res = await showModal({
        content: "是否要删除？"
      });
      // console.log(res);//confirm 为true 点击了确定   cancel 为 true 点击了取消
      if (res.confirm) {
        // 点击确定
        cart.splice(index, 1);
        this.setCart(cart);
